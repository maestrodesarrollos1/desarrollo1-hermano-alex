...
const Navbar = () => {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-[100] border-b border-[#C9E6D0] bg-white/[0.97] shadow-[0_10px_34px_rgba(15,61,46,0.12)] backdrop-blur-xl">
      <div ref={mainBarRef} className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between gap-4 px-4 py-2.5 sm:px-6 xl:px-6">
        ...
        <nav className="hidden flex-1 items-center justify-end gap-1 lg:flex" aria-label="Navegacion principal">
          ...
          <Link
            to={gamePath}
            className="font-nav ml-2 flex min-h-[44px] items-center justify-center border border-[#A8CFAE] bg-white px-3 text-center text-[10px] font-semibold uppercase tracking-[0.18em] text-[#123C2D] transition-colors hover:border-[#4F8B6D] hover:bg-[#4F8B6D] hover:text-white xl:text-[11px]"
          >
            Participa y gana
          </Link>
        </nav>
      ...
      >
        <div className="mx-auto flex max-w-7xl justify-center px-4 py-1.5 sm:px-6">
          <div className="font-nav flex flex-wrap items-center justify-center gap-x-2 gap-y-1 border border-[#DDEBDD] bg-white/92 px-3 py-1.5 text-[#123C2D] shadow-[0_8px_20px_rgba(18,60,45,0.08)] sm:px-4">
            <span className="text-[9px] font-semibold uppercase tracking-[0.3em] text-[#4F8B6D] sm:text-[10px]">
              Cuenta atras
            </span>

            {countdownItems.map((item) => (
              <span key={item.label} className="inline-flex items-center gap-2">
                <span className="text-[11px] leading-none text-[#DDEBDD]" aria-hidden="true">
                  |
                </span>
                <span className="inline-flex items-baseline gap-1 whitespace-nowrap">
                  ...
                  <span className="text-[9px] font-semibold uppercase tracking-[0.18em] text-[#4F8B6D] sm:text-[10px]">
      ...
      >
        <nav className="py-2" aria-label="Navegacion principal movil">
          ...
          <Link
            to={gamePath}
            onClick={() => setMobileOpen(false)}
            className="font-nav flex w-full items-center justify-center px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-[0.18em] text-[#0F3D2E] transition-colors hover:bg-[#DDEBDD] hover:text-[#123C2D]"
          >
            Participa y gana
          </Link>
        </nav>
